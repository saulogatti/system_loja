...
import 'package:system_loja/core/managers/usuario_manager.dart';
import 'package:system_loja/core/models/usuario.dart';

void main() {
  late UsuarioManager manager;
  late String testDataFile;

  setUp(() {
    // Cria arquivo temporário para os testes
    testDataFile = 'test/data/test_usuarios_${DateTime.now().millisecondsSinceEpoch}.json';
    manager = UsuarioManager(dataFile: testDataFile);
  });
  ...
  group('UsuarioManager - Operações CRUD', () {
    ...
    test('Deve atualizar um usuário existente', () async {
      ...
      expect(manager.usuarios.first.nivelPermissao, equals(NivelPermissao.administrador));
    });
  ...
  group('UsuarioManager - Segurança', () {
    test('Deve gerar hash SHA-256 da senha', () {
      ...
      expect(hash, hasLength(64)); // SHA-256 produz hash de 64 caracteres em hex
  ...
  group('UsuarioManager - Persistência', () {
    test('Deve salvar e carregar dados do arquivo JSON', () async {
      ...
      final novoManager = UsuarioManager(dataFile: testDataFile);
      ...
      expect(novoManager.usuarios.first.nivelPermissao, equals(NivelPermissao.administrador));
    });
  });
}
