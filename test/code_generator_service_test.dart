...
void main() {
  ...
  group('CodeGeneratorService - Product Code Generation', () {
    test('generateProductCode deve gerar código único no formato PRD-YYYYMMDD-NNNN', () async {
      final code = await codeGeneratorService.generateProductCode();
      
      expect(code, isNotNull);
      expect(code, startsWith('PRD-'));
      expect(code.length, greaterThanOrEqualTo(17)); // PRD-20260123-0001 = 17 chars
      
      // Verifica formato com regex
      final regex = RegExp(r'^PRD-\d{8}-\d{4}$');
      expect(regex.hasMatch(code), isTrue, reason: 'Código deve seguir formato PRD-YYYYMMDD-NNNN');
    });

    test('generateProductCode deve gerar códigos diferentes em sequência', () async {
    ...
    test('checkProductCodeExists deve retornar false para código inexistente', () async {
      final exists = await codeGeneratorService.checkProductCodeExists('CODIGO_NAO_EXISTE');
      
      expect(exists, isFalse);
    });

    test('checkProductCodeExists deve retornar true para código existente', () async {
  ...
  group('CodeGeneratorService - Product Code Validation', () {
    ...
    test('validateProductCode deve rejeitar código com menos de 3 caracteres', () async {
    ...
    test('validateProductCode deve rejeitar código com mais de 50 caracteres', () async {
      final longCode = 'A' * 51;
    ...
    test('validateProductCode deve aceitar código válido e único', () async {
      final result = await codeGeneratorService.validateProductCode('PRD-VALID-0001');
  ...
  group('CodeGeneratorService - Invoice Number Generation', () {
    test('generateInvoiceNumber deve gerar número único no formato NF-YYYYMMDD-NNNN', () async {
      final invoiceNumber = await codeGeneratorService.generateInvoiceNumber();
      
      expect(invoiceNumber, isNotNull);
      expect(invoiceNumber, startsWith('NF-'));
      expect(invoiceNumber.length, greaterThanOrEqualTo(16)); // NF-20260123-0001 = 16 chars
      
      // Verifica formato com regex
      final regex = RegExp(r'^NF-\d{8}-\d{4}$');
      expect(regex.hasMatch(invoiceNumber), isTrue, reason: 'Número deve seguir formato NF-YYYYMMDD-NNNN');
    });

    test('generateInvoiceNumber deve gerar números diferentes em sequência', () async {
      final invoiceNumber1 = await codeGeneratorService.generateInvoiceNumber();
      ...
      final invoiceNumber2 = await codeGeneratorService.generateInvoiceNumber();
      
      expect(invoiceNumber1, isNot(equals(invoiceNumber2)));
      expect(invoiceNumber2, startsWith('NF-'));
    });

    test('checkInvoiceNumberExists deve retornar false para número inexistente', () async {
      final exists = await codeGeneratorService.checkInvoiceNumberExists('NF-NAO-EXISTE');
      
      expect(exists, isFalse);
    });

    test('checkInvoiceNumberExists deve retornar true para número existente', () async {
      ...
      final exists = await codeGeneratorService.checkInvoiceNumberExists(invoiceNumber);
  ...
  group('CodeGeneratorService - Invoice Number Validation', () {
    ...
    test('validateInvoiceNumber deve rejeitar número com menos de 3 caracteres', () async {
    ...
    test('validateInvoiceNumber deve rejeitar número com mais de 50 caracteres', () async {
      final longNumber = 'A' * 51;
      final result = await codeGeneratorService.validateInvoiceNumber(longNumber);
    ...
    test('validateInvoiceNumber deve rejeitar número já existente', () async {
      ...
      final result = await codeGeneratorService.validateInvoiceNumber(invoiceNumber);
    ...
    test('validateInvoiceNumber deve aceitar número válido e único', () async {
      final result = await codeGeneratorService.validateInvoiceNumber('NF-VALID-0001');
