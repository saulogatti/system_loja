...
void main() {
  ...
  group('Integridade de Dados - Cliente', () {
    test('deve refletir alteração no nome do cliente nas notas fiscais', () async {
    ...
    test('deve refletir alteração no CPF do cliente nas notas fiscais', () async {
      // Arrange - Cria cliente
      final cliente = Cliente(
        id: 0,
        nome: 'Maria Santos',
  ...
  group('Integridade de Dados - Produto', () {
    test('deve refletir alteração no nome do produto nos itens da nota fiscal', () async {
      // Arrange - Cria cliente
      final cliente = Cliente(
        id: 0,
      ...
      expect(notaConsultada.itens[0].produtoNome, equals('Teclado Mecânico Premium'));
      expect(notaConsultada.itens[0].produtoCodigo, equals('TECLADO-001'));
    });

    test('deve refletir alteração no código do produto nos itens da nota fiscal', () async {
      // Arrange - Cria cliente
      final cliente = Cliente(
        id: 0,
        nome: 'Ana Costa',
      ...
      expect(notaConsultada.itens[0].produtoCodigo, equals('MONITOR-LED-001'));
    });
  });

  group('Integridade de Dados - Consultas Múltiplas', () {
    test('deve refletir dados atualizados em todas as consultas (por ID, número, cliente)', () async {
      // Arrange - Cria cliente
      final cliente = Cliente(
        id: 0,
        nome: 'Pedro Alves',
      ...
      final notaPorNumero = await notaFiscalManager.consultarPorNumero('NF-005');
      ...
      final notasPorCliente = await notaFiscalManager.buscarPorCliente(clienteId);
      ...
      expect(notasPorCliente[0].itens[0].produtoNome, equals('Impressora HP'));
